...
# =========================
# CONFIGURAÇÕES DE ESTIRO E CABEÇALHO
# =========================
PRIMARY = "#071B45"
BLUE = "#155EEF"
BLUE_DARK = "#0B3A75"
...
[data-testid="stTabs"] [data-baseweb="tab"] {
    height: 38px; min-height: 38px; padding: 0 18px; border-radius: 10px 10px 0 0;
    border: 1px solid #D7E6FA; border-bottom: 0; background: #FFFFFF; color: #667085; font-weight: 900;
    box-shadow: 0 6px 16px rgba(11,58,117,.06); transition: all .18s ease-in-out;
}
...
[data-testid="stTabs"] [data-baseweb="tab"][aria-selected="true"] {
    ...
    color: #FFFFFF !important; border-color: #155EEF !important; box-shadow: 0 10px 24px rgba(21,94,239,.22);
}
...
DISPLAY_NAMES = {
    ...
    "cd responsavel": "CD responsável", "transportador (grupo)": "Transportador (grupo)",
...
# =========================
# CARREGAMENTO INTELIGENTE EM LOTES (MÚLTIPLOS USUÁRIOS)
# =========================
@st.cache_data(show_spinner="Carregando dados operacionais... Aguarde.")
def carregar_dados_da_nuvem_ultra_ram(url):
    # Definindo tipos de dados estritos de antemão para reduzir o consumo de memória pela metade
    tipos_colunas = {
    ...
    lista_blocos = []
    # Carrega o CSV em blocos super otimizados ignorando colunas não usadas
    for bloco in pd.read_csv(url, sep=";", chunksize=50000, dtype=tipos_colunas, low_memory=False):
        # Normalização imediata das colunas lidas
        bloco.columns = [str(c).strip().strip('"').strip().lower() for c in bloco.columns]
        
        # Manter apenas o essencial para as métricas operacionais
        colunas_uteis = [c for c in bloco.columns if c in [
            ...
            'transportador (grupo)', 'situacao', 'pedido_gemco', 'prazo_cliente', 'realizado_cliente', 'cep_cliente'
        ]]
        lista_blocos.append(bloco[colunas_uteis])
        
    df = pd.concat(lista_blocos, ignore_index=True)
    
    # Tratamento numérico pós-combinação
...
def prep_data(df):
    ...
    # Substituição de lógicas pesadas por matrizes booleanas leves em int8
...
<div class="top-shell">
    <div style="display:flex; justify-content:space-between; align-items:center; gap:18px; flex-wrap:wrap;">
        <div>
            ...
            <div class="top-subtitle">Painel operacional de prazos por Geografia, Modal, CDs, Transportador e CEP.</div>
...
with f0: busca = st.text_input("Busca rápida", placeholder="Cidade, CEP, CD, Transportador...", key="txt_busca_main")
...
df = df_all
for col, val in [("geografia_comercial", geografia), ("modal", modal), ("uf cliente", uf), ("situacao", situacao), ("ecc", ecc), ("cd faturamento", cd_faturamento), ("cd responsavel", cd_responsavel), ("localizacao_comercial", localizacao), ("transportador (grupo)", transportador), ("cidade cliente", cidade)]:
...
if busca:
    df = df[df["cidade cliente"].str.contains(busca, case=False, na=False) | df["localizacao_comercial"].str.contains(busca, case=False, na=False) | df["transportador (grupo)"].str.contains(busca, case=False, na=False) | df["geografia_comercial"].str.contains(busca, case=False, na=False) | df["cep_cliente"].str.contains(busca, case=False, na=False)]
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["📌 Executivo", "🌎 Geografia", "🎯 SLA sugerido", "⏱️ Prazo x Realizado", "🚚 Transportador", "📍 CEP / Cidade"])

with tab1:
    c1, c2 = st.columns([1.1, 1])
    with c1:
        rank_loc = agg_metrics(df, ["geografia_comercial", "modal", "localizacao_comercial"])
        rank_loc = rank_loc[rank_loc["pedidos"] >= min_volume]
        if not rank_loc.empty: 
            fig = px.bar(rank_loc.head(15).sort_values("oportunidade"), x="oportunidade", y="localizacao_comercial", color="classe_acao", orientation="h", title="Top Localizações por Oportunidade", color_discrete_map={"Redução agressiva": GREEN, "Atacar agora": BLUE, "Testar redução": YELLOW, "Risco operacional": RED, "Monitorar": GRAY})
            fig.update_layout(paper_bgcolor="white", plot_bgcolor="white", height=420)
            st.plotly_chart(fig, width="stretch")
    with c2:
        fig = px.pie(pd.DataFrame({"Status": ["Antecipado", "No Prazo", "Atrasado"], "Pedidos": [antecipados, no_prazo, atrasados]}), names="Status", values="Pedidos", hole=.54, title="Composição do NS", color="Status", color_discrete_map={"Antecipado": CYAN, "No Prazo": GREEN, "Atrasado": RED})
        fig.update_layout(paper_bgcolor="white", height=360)
        st.plotly_chart(fig, width="stretch")

with tab2:
    st.dataframe(style_table(agg_metrics(df, ["geografia_comercial", "modal"]).head(50)), width="stretch")

with tab3:
    dim_label = st.selectbox("Dimensão", ["Geografia + Localização", "Geografia + Transportador", "Modal"], key="sb_dimensao_tab3")
    dim_map = {"Geografia + Localização": ["geografia_comercial", "localizacao_comercial"], "Geografia + Transportador": ["geografia_comercial", "transportador (grupo)"], "Modal": ["modal"]}
    rank = agg_metrics(df, dim_map[dim_label])
    st.dataframe(style_table(rank[rank["pedidos"] >= min_volume].head(50)), width="stretch")

with tab4:
    lead = df.groupby(["geografia_comercial", "modal", "prazo_cliente", "realizado_cliente"]).agg(pedidos=("pedido_gemco", "nunique"), oportunidade=("oportunidade", "sum")).reset_index()
    st.dataframe(style_table(lead.sort_values("oportunidade", ascending=False).head(50)), width="stretch")

with tab5:
    lt = agg_metrics(df, ["geografia_comercial", "transportador (grupo)"])
    st.dataframe(style_table(lt[lt["pedidos"] >= min_volume].head(50)), width="stretch")

with tab6:
    cidade_df = agg_metrics(df, ["uf cliente", "cidade cliente"])
    st.dataframe(style_table(cidade_df[cidade_df["pedidos"] >= min_volume].head(50)), width="stretch")
